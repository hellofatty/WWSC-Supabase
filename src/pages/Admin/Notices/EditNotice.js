...
export default function EditNotice({ notice, toggle }) {
    ...
    const [shortTitle, setShortTitle] = useState(notice.shortTitle);
    const [content, setContent] = useState(notice.content);
    const [excerpt, setExcerpt] = useState(notice.excerpt);

    const [titleTw, setTitleTw] = useState(notice.titleTw);
    const [shortTitleTw, setShortTitleTw] = useState(notice.shortTitleTw);
    const [contentTw, setContentTw] = useState(notice.contentTw);
    const [excerptTw, setExcerptTw] = useState(notice.excerptTw);

    const [titleCn, setTitleCn] = useState(notice.titleCn);
    const [shortTitleCn, setShortTitleCn] = useState(notice.shortTitleCn);
    const [contentCn, setContentCn] = useState(notice.contentCn);
    const [excerptCn, setExcerptCn] = useState(notice.excerptCn);
    // const [lang, setLang] = useState(notice.lang);

    const [status, setStatus] = useState(notice.status);

    const convertedPostedDate = moment(notice.postedDate.toDate().toDateString()).format("yyyy-MM-DD");
    const [postedDate, setPostedDate] = useState(convertedPostedDate);

    const { updateDocument, response } = useFirestore(`notices`);
