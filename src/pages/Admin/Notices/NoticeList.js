...
export default function NoticeList() {
    ...
    const { documents: notices, error } = useCollection("notices", ["author", "==", "WWSC"], ["postedDate", "desc"]);

    const { deleteDocument } = useFirestore("notices");
    ...
    let arryNoticesYears = [];

    for (let i = 0; i < notices?.length; i++) {
        arryNoticesYears.push(notices[i].year);
    }
    ...
    const arryNoticesYearsNoDup = removeDuplicates(arryNoticesYears);
    console.log(arryNoticesYearsNoDup);

    const listByFilter = notices?.filter((notice) => {
        return query.toLowerCase() === "all"
        ? notices: notice.year.toLowerCase().includes(query)
                ...
                <div>
                    <Modal isOpen={modal} toggle={toggle} backdrop="static" size="lg" centered={true}>
                        <ModalHeader toggle={toggle} close={closeBtn}>
                            <h2>{t("admin.notice-list.add-new-notice-title")}</h2>
                        </ModalHeader>
                        <ModalBody>
                            <AddNotice toggle={toggle} />
                        </ModalBody>
                    </Modal>
                </div>
                                            ...
                                            <div className="notice-list-icon">
                                                ...
                                                <span>
                                                    ...
                                                    >
                                                        <DeleteIcon
                                                            fontSize="small"
                                                            onClick={() => deleteDocument(notice.id)}
                                                            style={{ color: "gray" }}
                                                        />
                                                    </Tooltip>
                                                </span>
                                ...
                                {notices &&
                                    listByFilter.splice(0, 40).map((notice) => (
                                        <li className="recent-notice-item" key={notice.id}>
                                            {currentLang === "en" && (
                                                <Link to={`/admin-zone/notice/${notice.id}`}>{notice.shortTitle}</Link>
                                            )}
                                            {currentLang === "zh-TW" && (
                                                <Link to={`/admin-zone/notice/${notice.id}`}>
                                                    {notice.shortTitleTw}
