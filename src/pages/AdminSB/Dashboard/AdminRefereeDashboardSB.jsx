...
export default function AdminRefereeDashboardSB() {
    ...
    // Filter referees by grade
    const G3G4RefereeList = refereeList?.filter((referee) => referee.grade === "3" || referee.grade === "4");
    const G3refereeList = refereeList?.filter((referee) => referee.grade === "3");
    const G4refereeList = refereeList?.filter((referee) => referee.grade === "4");
    const RITList = refereeList?.filter((referee) => referee.grade === "0");

    // Calculate totals
    const totalReferees = G3G4RefereeList?.length || 0;
    const totalG3Referees = G3refereeList?.length || 0;
    const totalG4Referees = G4refereeList?.length || 0;
    const totalRITs = RITList?.length || 0;
    ...
    // Process country data for pie charts
    const processCountryData = (refereeList, countryKey) => {
        if (!refereeList) return [];

        const countryGroups = refereeList.reduce((acc, referee) => {
            const countryLabel = referee[countryKey]?.label;
            if (countryLabel) {
        ...
        return Object.entries(countryGroups)
            .filter(([label]) => label) // Remove entries with null/undefined labels
    ...
    const pieChartData = {
        en: processCountryData(G3G4RefereeList, "country_en"),
        tw: processCountryData(G3G4RefereeList, "country_tw"),
        cn: processCountryData(G3G4RefereeList, "country_cn"),
    };
                    ...
                    <Stack direction={{ xs: "column", sm: "row" }} spacing={{ xs: 1, sm: 2, md: 2 }}>
                        <Item>
                            ...
                            <div className="item-more-info">
                                <Link to="/admin-zone-SB/referee-list">
                        ...
                        <Item style={{ background: "brown", color: "white" }}>
                            ...
                            <div className="item-more-info">
                                <Link to="/admin-zone-SB/G3-referee-list">
                        ...
                        <Item style={{ background: "gold", color: "black" }}>
                            ...
                            <div className="item-more-info">
                                <Link to="/admin-zone-SB/G4-referee-list">
                        ...
                        <Item style={{ background: "green", color: "white" }}>
                            ...
                            <div className="item-more-info">
                                <Link to="/admin-zone-SB/referee-in-training-list">
