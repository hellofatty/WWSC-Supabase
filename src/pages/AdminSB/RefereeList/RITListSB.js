...
export default function RITListSB() {
    // All referees
    const { t } = useTranslation("global");
    const [RITList, setRITList] = useState(null);
    ...
    // Fetch RIT referees from Supabase
    useEffect(() => {
        const fetchRITs = async () => {
            try {
                ...
                console.error("Error fetching RITs:", err);
    ...
    const handleDelete = async (id) => {
        try {
            ...
            // Update local state
            setRITList((prev) => prev.filter((ref) => ref.id !== id));
    ...
    const listByFilter = RITList
        ? RITList.filter((rit) => {
              switch (filter) {
                  ...
                  case "pending":
                  case "inactive":
                      console.log(rit.status, filter);
                      return rit.status === filter;
                  default:
                      return true;
              }
          })
                    ...
                    <div>
                        <div style={{ display: "flex", alignItems: "center" }}>
                            <span style={{ marginLeft: "12px", marginBottom: "12px" }}>
                                <ContactPageIcon style={{ fontSize: "2.6rem", color: "Teal" }} />
                            </span>
                            <div className="page-primary-title" style={{ marginLeft: "6px" }}>
                                {t("admin.referee-list.RIT-title")}
                            </div>
                        </div>
                        <div className="referee-list-filter-bar">
                            <div className="referee-list-select-container">
                                ...
                                >
                                    <option value="all">
                                        <span className="grade-item">{t("admin.referee-list.all-status")}</span>
                                    </option>
                                    <option value="active">
                                        ...
                                        <TableHead>
                                            <TableRow style={{ backgroundColor: "grey" }}>
                                                ...
                                                <TableCell
                                                    ...
                                                    style={{ color: "white", whiteSpace: "nowrap", width: "50px" }}
                                                >
                                                    {t("admin.referee-list.othername")}
                                                </TableCell>
                                                <TableCell
                                                    align="center"
                                                    style={{ color: "white", whiteSpace: "nowrap", width: "50px" }}
                                                ...
                                                currentRows
                                                    ...
                                                    .map((referee) => (
                                                        ...
                                                        >
                                                            ...
                                                            <TableCell align="center">{referee.other_name}</TableCell>
